...
def modificar_factura(pdf_bytes, export_type, new_logo, new_company_name, invoice_value_text):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    max_width = 370  # Ancho máximo antes de hacer un salto de línea
    line_spacing = 14  # Espaciado entre líneas
    
    for page in doc:
        # Reemplazar razón social exactamente en la misma posición
        text_instances = page.search_for("INDUSTRIAS FLYBB SAS")
        for inst in text_instances:
            rect = fitz.Rect(inst.x0, inst.y0, inst.x1, inst.y1)
            page.draw_rect(rect, color=(1,1,1), fill=(1,1,1), overlay=True)
            page.insert_text((inst.x0, inst.y1), new_company_name, fontsize=12, color=(0, 0, 0), fontname="helvetica-bold")
        ...
        for inst in text_instances:
            ...
            # Insertar el valor en letras con el mismo ancho y posición adecuada
        ...
        # Reemplazar SOLO TOTAL USD por el tipo de exportación sin afectar SUBTOTAL USD, en la misma ubicación pero un poco más arriba
        ...
        for inst in text_instances:
            if not any(sub_inst.y0 == inst.y0 and "SUBTOTAL USD" in page.get_text("text") for sub_inst in page.search_for("SUBTOTAL USD")):
                rect = fitz.Rect(inst.x0, inst.y0, inst.x1, inst.y1)
                page.draw_rect(rect, color=(1,1,1), fill=(1,1,1), overlay=True)
                page.insert_text((inst.x0, inst.y1), export_type, fontsize=9, color=(0, 0, 0), fontname="helvetica-bold")
...
if uploaded_file and export_type and new_company_name and invoice_value_text:
    if st.button("Modificar Factura"):
        modified_pdf = modificar_factura(uploaded_file.read(), export_type, new_logo, new_company_name, invoice_value_text)
